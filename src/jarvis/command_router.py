...
class CommandRouter:
    ...
    def handle(self, command: str) -> str | dict[str, str] | None:
        ...
        if self._is_wallpaper_request(lower):
            ...
            if not path:
                return "Indícame la ruta de la imagen, por ejemplo: cambia el fondo a C:\\Users\\Usuario\\Pictures\\fondo.jpg"
        ...
        # Computer Use explícito: observar o controlar la pantalla de forma visible.
        if lower in {"mira la pantalla", "observa la pantalla", "captura la pantalla", "analiza la pantalla"}:
            obs = self.computer.observe()
            return f"Pantalla observada: {obs.width}x{obs.height}. {obs.note}"
    ...
    def _provider_intent(text: str) -> str | None:
        if re.search(r"\b(?:usa|usar|cambia(?:r)?(?:\s+a)?|cámbiate a|cambiar a|selecciona)\s+(?:el\s+)?(?:modelo\s+)?(?:gemini)\b", text):
            return "gemini"
    ...
    def _is_confirmation(text: str) -> bool:
        ...
        return normalized in {"sí", "si", "si envialo", "sí envíalo", "sí envía", "si envia", "envíalo", "envialo", "envialo ya", "ennvialo", "envía", "envia", "mándalo", "mandalo", "mándale", "mandale", "hazlo", "confirmo", "confirmar", "dale", "adelante", "procede", "proceder", "sí hazlo", "si hazlo"}
    ...
    def _teams_intent(self, text: str) -> dict[str, str] | None:
        ...
        has_contact = bool(re.search(r"\b(?:a|para)\s+[^,;]+", text, re.IGNORECASE) or re.search(r"\b(?:contacto|persona|se llama)\s+[^,;]+", text, re.IGNORECASE))
    ...
    def _extract_contact_and_message(text: str) -> tuple[str, str]:
        ...
        cleaned = re.sub(r"\b(?:personal|educativo|educativa|colegio|escuela|institucional|en google|en el google|por google|del google|de google|en chrome|en el navegador|desde google|desde chrome|navegador)\b", " ", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r"\b(?:entra|entrar|abre|abrir|mira|mirar|busca|buscar|ve|ir|entra a)\b", " ", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r"\b(?:a los que les he escrito|a los que les e escrito|a los que les escribí|a los q(?:ue)? les he escrito|a los q(?:ue)? les e escrito|a quien le escribí|a quien le escribi|mis chats|mis conversaciones)\b", " ", cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r"\s+", " ", cleaned).strip(" ,:-")
        named = re.search(r"\b(?:la|el)?\s*(?:que\s+)?se\s+llama\s+(.+?)(?=\s+(?:y\s+)?(?:dile|escr[ií]bele|env[ií]ale|m[aá]ndale)\b|$)", cleaned, flags=re.IGNORECASE)
        if named:
            person = named.group(1).strip(" ,:-\""); after = cleaned[named.end():]
            body_match = re.search(r"\b(?:y\s+)?(?:dile(?:\s+que)?|dile\s+esto|escr[ií]bele|env[ií]ale|m[aá]ndale)\s*[:,-]?\s*(.+)$", after, flags=re.IGNORECASE)
            return person, body_match.group(1).strip(" \"'") if body_match else ""
        direct = re.search(r"\b(?:a|para)\s+(.+?)(?=\s+(?:y\s+)?(?:dile|escr[ií]bele|env[ií]ale|m[aá]ndale)\b|$)", cleaned, flags=re.IGNORECASE)
        if direct:
            person = direct.group(1).strip(" ,:-\""); after = cleaned[direct.end():]
            body_match = re.search(r"\b(?:y\s+)?(?:dile(?:\s+que)?|dile\s+esto|escr[ií]bele|env[ií]ale|m[aá]ndale)\s*[:,-]?\s*(.+)$", after, flags=re.IGNORECASE)
            body = body_match.group(1).strip(" \"'") if body_match else ""
            if person.lower() not in {"la", "el", "que", "los", "las", "los q", "q"}:
                return person, body
        contact = re.search(r"\b(?:contacto|persona)\s+(.+?)(?=\s+(?:y\s+)?(?:dile|escr[ií]bele|env[ií]ale|m[aá]ndale)\b|$)", cleaned, flags=re.IGNORECASE)
        if contact:
            person = contact.group(1).strip(" ,:-\""); after = cleaned[contact.end():]
            body_match = re.search(r"\b(?:y\s+)?(?:dile(?:\s+que)?|dile\s+esto|escr[ií]bele|env[ií]ale|m[aá]ndale)\s*[:,-]?\s*(.+)$", after, flags=re.IGNORECASE)
            return person, body_match.group(1).strip(" \"'") if body_match else ""
        return "", ""
    ...
    def _communication_intent(text: str) -> dict[str, str] | None:
        lower = text.lower()
        if "teams" in lower or not re.search(r"\b(?:escr[ií]bele|env[ií]ale|m[aá]ndale|env[ií]a|manda|escribirle|mandar)\b", lower):
            return None
        if "gmail" in lower or "correo" in lower or "email" in lower:
            return {"communication": "gmail", "person": "el contacto", "body": "", "url": "https://mail.google.com/", "message": "Abrí Gmail en el navegador. La automatización de Gmail todavía requiere configurar su herramienta específica."}
        return None
    ...
    def system_status() -> str:
        memory = psutil.virtual_memory(); cpu = psutil.cpu_percent(interval=0.2)
    ...
    def open_target(target: str) -> str:
        original = target.strip(); clean = re.sub(r"^(?:en\s+)?(?:el\s+)?(?:navegador|google|chrome)\s+", "", original.lower()).strip(); clean = re.sub(r"^(?:la|el)\s+aplicación\s+", "", clean).strip()
        if re.match(r"^https?://", clean, flags=re.IGNORECASE):
            webbrowser.open(clean, new=2); return f"Abriendo {original} en el navegador."
        if "teams" in clean:
            educational = bool(re.search(r"\b(?:educativo|educativa|colegio|escuela|institucional)\b", clean)); webbrowser.open("https://teams.microsoft.com/" if educational else "https://teams.live.com/v2/", new=2); return f"Abriendo Teams {'educativo' if educational else 'personal'} en el navegador."
        ...
        if clean in websites:
            webbrowser.open(websites[clean], new=2); return f"Abriendo {original}."
