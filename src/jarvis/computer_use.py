...
class ComputerUse:
    ...
    La captura solo ocurre cuando JARVIS la solicita explícitamente.
    """
    ...
    def observe(self) -> ScreenObservation:
        ...
        try:
            ...
            return ScreenObservation(width, height, encoded, "Captura puntual realizada.")
    ...
    def type_text(self, text: str) -> str:
        ...
        if pyperclip is not None and any(ord(char) > 127 for char in text):
            ...
            return "Texto escrito mediante pegado visible."
    ...
    def hotkey(self, *keys: str) -> str:
        if not self.enabled:
            return "Control visual no disponible: instala pyautogui."
        allowed = {"ctrl", "shift", "alt", "tab", "enter", "esc", "space", "backspace", "home", "end", "up", "down", "left", "right", "win"}
        if not all(k.lower() in allowed or (len(k) == 1 and ord(k) < 128) for k in keys):
